...
export default class MetadataWranglerModal extends Modal {
	...
	onOpen() {
		...
		const createEditSettings = () => {
			...
			editProperty.setName(`Edit Property`).addDropdown((dropdown) => {
				...
				const options = _.fromPairs(
					_.uniq(
						files.map((x) => _.keys(_.omit(x, 'file'))).flat()
					).map(
						(x: string) =>
							[x.toLowerCase(), x.toLowerCase()] as [
								string,
								string
							]
					)
		...
		const createEditTagSettings = () => {
			...
			editTag.addText((text) => {
				text.onChange((value) => {
					...
					if (currentAddTagValue)
						delete this.edits.tag[currentAddTagValue]
			...
			this.displayTags = contentEl.appendChild(
				document.createElement('div')
			)
		}
		...
		new Setting(contentEl)
			...
			.addToggle((toggle) =>
				toggle.onChange(
					(value) => (this.options.convertToLowercase = value)
				)
			)
		...
		new Setting(contentEl)
			...
			.addToggle((toggle) =>
				toggle.onChange(
					(value) => (this.options.syncLinksInMetadata = value)
				)
			)
	...
	async process() {
		const {
			syncLinksInMetadata: syncingLinksInMetadata,
			convertToLowercase,
		} = this.options
				...
				app.fileManager.processFrontMatter(
					...
					(frontmatter: Record<string, any>) => {
						for (let property of _.keys(frontmatter)) {
							frontmatter[property.toLowerCase()] =
				...
				app.vault.process(thisFile, (text) => {
					...
					const syncLinksInMetadata = () => {
						...
						for (let propertyListedLinks of propertyMatches) {
							const lines = propertyListedLinks
								.split('\n')
								.filter((x) => x)
							let propName = lines[0]
								.match(/\w+/)?.[0]
								.toLowerCase()
							invariant(propName)
